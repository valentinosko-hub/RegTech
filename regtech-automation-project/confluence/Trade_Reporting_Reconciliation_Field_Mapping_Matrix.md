...
| ASIC | ASIC | Trade | `ASIC2_Transactions.UTI` | Deterministic in SP: `'549300OK2V4QF20B0D04' + 'P' + PositionID + side + 'A'` from `Type` mapping | Payload UTI | DTCC/TR UTI | UTI | Exact match | T+2 | Critical | Reg Ops | Procedure-validated (ASIC transactions SP) | UTI side suffix driven by buy/sell branch logic |
...
| ASIC | ASIC | Trade | `ASIC2_Transactions.CDE_Counterparty_2`, `Counterparty_2_name`, `Country_of_counterparty_2` | Derived from account-type/player-level/LEI branches using `#cust`, `ASIC2_Customer_PositionReport`, fallback from prior `ASIC2_Transactions` by CID | Payload CP2 identity/name/country | TR CP2 validation fields | UTI + CID | Exact regex + value mapping | T+2 | Critical | Compliance Data + Reg Ops | Procedure-validated (ASIC transactions SP) | Includes DSR logic for blank-name/country handling and fallback restoration |
...
| ASIC | ASIC | Trade | `ASIC2_Transactions.CDE_Price`, `CDE_Price_currency`, `CDE_Price_notation` | Derived from execution price and instrument-type formatting rules in SP; FX/index branches use alternate precision/notation | Payload price fields | DTCC/TR price validation | UTI + instrument | Numeric tolerance class P1 | T+2 | Warning | Reg Ops + Trading Tech | Procedure-validated (ASIC transactions SP) | Includes `CDE_Price_notation` branch (`1` vs `3`) and GBX conversion handling |
