...
| Model | Regulation | Reporting object | Reporting field (expected) | Source system/table.field (expected origin) | Submitted evidence field | Actual evidence field | Reconciliation key(s) | Match rule / tolerance | Timeliness | Severity | Owner | Status | Notes |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| EMIR | EMIR UK REFIT | Trade/Lifecycle | `EMIR3_UK_Refit_Report_Daily.UTI` | Derived in SP from `#all` + previous-day `EMIR3_UK_Refit_Report` (`ERP.UTI`) | Cappitech TR payload UTI | REGIS/DTCC response UTI | UTI | Exact match | T+1 | Critical | Reg Ops + Data Eng | Procedure-validated | Generated using CID/Instrument/date pattern when missing in prior day |
| EMIR | EMIR UK REFIT | Trade/Lifecycle | `EMIR3_UK_Refit_Report_Daily.Action_type` | SP logic from `#all.Trade`, `OpenORClose`, and lifecycle state | Payload action type | TR action/status | UTI + event date | Exact enum with transition matrix | T+1 | Critical | Reg Ops | Procedure-validated | Position rows generate `PSTN`; trade rows generate `TCTN`/`POSC` logic |
| EMIR | EMIR UK REFIT | Lifecycle | `EMIR3_UK_Refit_Report_Daily.Execution_timestamp` | From previous-day report (`ERP.Execution_timestamp`) or current event timestamp from `#all.Occurred` | Payload execution timestamp | TR accepted execution timestamp | UTI + action | Exact timestamp (UTC normalization) | T+1 | Critical | Reg Ops | Procedure-validated | Uses fallback strategy if prior-day UTI state exists |
| EMIR | EMIR UK REFIT | Lifecycle | `EMIR3_UK_Refit_Report_Daily.Confirmation_timestamp` | From previous-day report (`ERP.Confirmation_timestamp`) else from `#all.Occurred` | Payload confirmation timestamp | TR confirmation/validation timestamp | UTI | Exact timestamp (UTC normalization) | T+1 | Critical | Reg Ops | Procedure-validated | Explicit fallback in SP |
| EMIR | EMIR UK REFIT | Trade | `EMIR3_UK_Refit_Report_Daily.Counterparty_1` | From `EMIR2_Customer` + UK client details (`#emir_UK_clients_details`) and LEI logic | Payload CP1 identifier | TR counterparty validation | UTI | Exact regex + value match | T+1 | Critical | Compliance Data + Reg Ops | Procedure-validated | Conditional LEI override logic for specific CIDs |
| EMIR | EMIR UK REFIT | Trade | `EMIR3_UK_Refit_Report_Daily.Counterparty_2` | Constant/reporting-entity logic + UK client details | Payload CP2 identifier | TR counterparty validation | UTI | Exact match | T+1 | Critical | Compliance Data + Reg Ops | Procedure-validated | CP2 fixed to reporting entity in main branch of logic |
| EMIR | EMIR UK REFIT | Trade | `EMIR3_UK_Refit_Report_Daily.Nature_of_counterparty_1` | Derived using `#emir_UK_clients_details.financial_counterparty_y_n_` and account-type logic | Payload CP1 nature | TR validation field | UTI | Exact enum mapping | T+1 | Critical | Compliance + Reg Ops | Procedure-validated | Uses `'F'/'N'` mappings |
| EMIR | EMIR UK REFIT | Trade | `EMIR3_UK_Refit_Report_Daily.Nature_of_counterparty_2` | Derived from account/LEI/UK client details with fallback branches | Payload CP2 nature | TR validation field | UTI | Exact enum mapping | T+1 | Critical | Compliance + Reg Ops | Procedure-validated | Multi-branch case logic |
| EMIR | EMIR UK REFIT | Trade | `EMIR3_UK_Refit_Report_Daily.UPI` | Joined from `[ThirdParty_Fivetran].[Fivetran].[regtech].[emir_refir_upi]` by InstrumentID | Payload UPI | TR UPI validation/status | UTI + InstrumentID | Exact match | T+1 | Critical | Reg Data Management | Procedure-validated | Direct table join in SP |
| EMIR | EMIR UK REFIT | Trade | `EMIR3_UK_Refit_Report_Daily.Product_classification` | SP case logic by `InstrumentTypeID`, `SettlementTypeID`, and instrument lists | Payload product classification | TR product-class validation | UTI + UPI | Exact/allowed mapping set | T+1 | Warning | Reg Data Management | Procedure-validated | Includes DSR updates for new instruments (557/558/559) |
| EMIR | EMIR UK REFIT | Trade | `EMIR3_UK_Refit_Report_Daily.Base_product` / `Sub_product` / `Further_sub_product` | SP case logic mapped by InstrumentID and type | Payload base/sub/further sub product | TR classification validation | UTI + InstrumentID | Exact mapping by approved code sets | T+1 | Warning | Reg Data Management | Procedure-validated | Commodity mapping enriched repeatedly in change log |
| EMIR | EMIR UK REFIT | Valuation/Collateral | `EMIR3_UK_Refit_Report_Daily.Valuation_amount`, `Price`, `Notional_amount_of_leg_1`; `EMIR3_UK_Refit_Report_Collateral.Variation_margin_posted_by_counterparty_1_post_haircut`, `Excess_collateral_posted_by_counterparty_1`, `Collateral_portfolio_code` | Daily valuation/notional built in `#all`; collateral built in `#EMIR3_collateral_calculation_agg` from `#equity` (`EMIR2_ext_DWH_V_Liabilities`) + eligible IDs from `EMIR2_Position`/`EMIR2_Customer` | Payload valuation/notional/collateral fields | TR valuation/collateral response fields | UTI + valuation date; collateral portfolio code + report date | Numeric tolerance classes V1/N1/P1/C1 | T+1 | Warning/Critical | Reg Ops + Finance Control + Collateral Ops | Procedure-validated | Includes collateral `MARU` action and USD collateral amount outputs from collateral SP |
...
| EMIR | 12 | 12 | Populated (procedure-validated, endpoint validation pending) | Confirm REGIS/DTCC response code mapping and nullable behaviors; lock payload/response physical names |
