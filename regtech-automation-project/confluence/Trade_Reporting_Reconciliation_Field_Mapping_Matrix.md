...
| ASIC | ASIC (client + hedge + aggregate) | Trade/Position | `ASIC2_Transactions.UTI`; `ASIC2_Transactions_Hedge.UTI`; `ASIC2_Positions_AGG.UTI` | Transactions SP builds deterministic client UTI (`...P...A`); hedge SP rewrites `P->H`; aggregate SP generates CID+instrument+date UTI with switch-date branch and excluded-UTI overrides, then applies targeted manual remediation updates for known rejected dates | Cappitech ASIC payload UTI | DTCC/TR UTI | UTI | Exact match | T+2 | Critical | Reg Ops | Procedure-validated (ASIC transactions + hedge + position agg SPs) | Covers client trade UTI, hedge mirror UTI, and aggregate position UTI continuity |
...
| ASIC | ASIC (client + hedge) | Trade | `ASIC2_Transactions.CDE_Counterparty_2`, `Counterparty_2_name`, `Country_of_counterparty_2`; `ASIC2_Transactions_Hedge.CDE_Counterparty_2` | Base SP derives client CP2 from account-type/player-level/LEI branches with name/country fallback; hedge SP overrides CP2 to fixed LEI `213800GIFQMSV7HROS23`, forces identifier type `TRUE`, and blanks name/country | Payload CP2 identity/name/country | TR CP2 validation fields | UTI + CID | Exact regex + value mapping | T+2 | Critical | Compliance Data + Reg Ops | Procedure-validated (ASIC transactions + hedge SPs) | Hedge side intentionally suppresses CP2 name/country fields |
...
| ASIC | ASIC | Position | `ASIC2_Positions.[Volume]`, `ASIC2_Positions.[Open Price]`, `ASIC2_Positions.[Close Price]`; `ASIC2_Positions_AGG.Quantity`, `OpenPrice`, `ClosePrice_EOD`, `CDE_Total_notional_quantity_of_leg_1` | Position SP builds open-position EOD snapshot from `ASIC2_ext_OpenPositions_PositionsReport` + EOD prices; aggregate SP nets signed units by CID/instrument, maintains SCD open state, and outputs aggregate quantity/open/close values with zero-quantity handling (`for_update`) | Submitted position quantity/price fields | DTCC/TR position quantity/price ack | `PositionID` or aggregate UTI + instrument + date | Numeric tolerance class Q1/P1 | T+2 | Warning/Critical | Reg Ops + Position Control | Procedure-validated (ASIC position + position agg SPs) | Includes side-aware close pricing and aggregate net-position behavior |
| ASIC | ASIC | Valuation | `ASIC2_Positions.[ValuationDateTime]`; `ASIC2_Positions_AGG.CDE_Valuation_timestamp`, `CDE_Valuation_amount`, `CDE_Valuation_currency`, `CDE_Valuation_method` | Position SP provides valuation timestamp and price basis; aggregate SP computes explicit MTM (`side-aware quantity * (ClosePrice_EOD-OpenPrice) * USDConversionRate`) and stamps `MTMA`/`USD` valuation outputs | Submitted valuation datetime/amount context | DTCC/TR valuation amount/time fields | `PositionID` or aggregate UTI + valuation timestamp | Numeric tolerance class V1 | T+2 | Warning | Reg Ops + Finance Control | Procedure-validated (ASIC position + position agg SPs) | Aggregate flow carries explicit valuation amount, while base position flow remains reconstruction-based |
