...
| ASIC | ASIC (client + hedge) | Trade | `ASIC2_Transactions.UTI`; `ASIC2_Transactions_Hedge.UTI` | Client UTI deterministic in base SP: `'549300OK2V4QF20B0D04' + 'P' + PositionID + side + 'A'`; hedge SP clones same-day open rows and transforms UTI via `REPLACE(UTI,'P','H')` | Cappitech ASIC payload UTI | DTCC/TR UTI | UTI | Exact match | T+2 | Critical | Reg Ops | Procedure-validated (ASIC transactions + hedge SPs) | Hedge flow emits H-prefixed UTI from base client records |
...
| ASIC | ASIC | Position | `ASIC2_Positions.[Volume]`, `ASIC2_Positions.[Open Price]`, `ASIC2_Positions.[Close Price]` | Built in `SP_ASIC2_PositionReport` from open-position snapshot (`ASIC2_ext_OpenPositions_PositionsReport`) joined to EOD prices (`Reg_Ext_CurrencyPriceMaxDateWithSplit`) with GBX normalization via `#Metadata`/`#Prices_EOD` | Submitted position quantity/price fields | DTCC/TR position quantity/price ack | `PositionID` + instrument + date | Numeric tolerance class Q1/P1 | T+2 | Warning/Critical | Reg Ops + Position Control | Procedure-validated (ASIC position SP) | Position set includes only open trades at valuation cut; close price chosen by side (`Ask` for sell, `Bid` for buy) |
| ASIC | ASIC | Valuation | `ASIC2_Positions.[ValuationDateTime]`, implied MTM from `Volume * (Close Price - Open Price)` | `SP_ASIC2_PositionReport` stamps valuation time from `#Prices_EOD.Occurred` and provides open/close price basis for position MTM reconstruction | Submitted valuation datetime/amount context | DTCC/TR valuation amount/time fields | `PositionID` + valuation datetime | Numeric tolerance class V1 | T+2 | Warning | Reg Ops + Finance Control | Procedure-validated (ASIC position SP) | SP does not emit explicit valuation amount column; valuation is reconstructed from emitted price/volume fields |
