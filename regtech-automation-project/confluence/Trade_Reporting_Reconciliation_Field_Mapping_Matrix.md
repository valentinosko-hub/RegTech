...
| Model | Regulation | Reporting object | Reporting field (expected) | Source system/table.field (expected origin) | Submitted evidence field | Actual evidence field | Reconciliation key(s) | Match rule / tolerance | Timeliness | Severity | Owner | Status | Notes |
|---|---|---|---|---|---|---|---|---|---|---|---|---|---|
| EMIR | EMIR EU/UK REFIT (+ Seychelles) | Trade/Lifecycle | `EMIR2_Refit_Report_Daily.UTI` / `EMIR3_UK_Refit_Report_Daily.UTI` / `EMIR2_ETORO_Refit_Positions.UTI` / `EMIR2_ETORO_Refit_Trades.UTI` / `EMIR2_Seychelles_Refit_Report_Daily.UTI` | Derived in SPs from `#all` + prior-day refs (`EMIR2_Refit_Report` / `EMIR3_UK_Refit_Report`) and deterministic ETORO/Seychelles patterns; Seychelles uses `HN`/`HP` branches with LEI prefix and regulation filter (`RegulationID=9`) | Cappitech TR payload UTI | REGIS/DTCC response UTI | UTI | Exact match | T+1 | Critical | Reg Ops + Data Eng | Procedure-validated (EU+UK+ETORO positions+trades+Seychelles) | EU/UK reuse prior-day UTI; ETORO flows use AUSHN/AUSHP; Seychelles flow uses HN/HP constructs and prev-day fallback |
| EMIR | EMIR EU/UK REFIT (+ Seychelles) | Trade/Lifecycle | `EMIR2_Refit_Report_Daily.Ticket`, `Report_tracking_number`; `EMIR2_ETORO_Refit_Positions.Ticket`; `EMIR2_ETORO_Refit_Trades.Ticket`; `EMIR2_Seychelles_Refit_Report_Daily.Ticket` | Built in EU SP from `#all` with fallback to previous-day `ERP.Ticket`; ETORO positions/trades use deterministic ticket patterns (`AUSHN`/`AUSHP`); Seychelles uses `HN+InstrumentID+Date+E` for positions and `HP+PositionID+side+E` for trades | Payload ticket/tracking refs | TR response reference IDs | UTI + ticket/tracking ref | Exact match | T+1 | Critical | Reg Ops | Procedure-validated (EU+ETORO positions+trades+Seychelles) | Includes Seychelles `Report_tracking_number` branch and `Subsequent_position_UTI` linkage for trade rows |
...
| EMIR | EMIR EU/UK REFIT (+ Seychelles) | Trade taxonomy | `UPI`, `Isda_taxonomy`, `Anna_underlying_structure`, `Anna_reference_rate`, `Anna_underlying_index` (EU + ETORO + Seychelles flows) and `UPI` (UK) | Joined from Fivetran taxonomy/UPI tables by InstrumentID in EU daily, Seychelles daily, and both ETORO procedures; UK SP directly maps UPI | Payload UPI/taxonomy fields | TR UPI/classification validation | UTI + InstrumentID | Exact match | T+1 | Critical | Reg Data Management | Procedure-validated (EU+UK+ETORO positions+trades+Seychelles) | Seychelles daily includes taxonomy fallback placeholder when mapping missing |
...
| EMIR | EMIR EU/UK REFIT (+ Seychelles) | Valuation/Notional | `EMIR2_Refit_Report_Daily.Valuation_amount`, `Price`, `Notional_amount_of_leg_1`; `EMIR3_UK_Refit_Report_Daily.Valuation_amount`; `EMIR2_ETORO_Refit_Positions.Valuation_amount`; `EMIR2_ETORO_Refit_Trades.Notional_amount_of_leg_1`; `EMIR2_Seychelles_Refit_Report_Daily.Valuation_amount` | Built from `#all` + `#PricesEOD` for EU/UK/Seychelles; ETORO positions SP recalculates valuation from `#RE_AGG_ASIC_Positions`; ETORO trades SP populates notional and leaves valuation empty for TCTN rows; Seychelles computes valuation for `PSTN` rows in final insert with direction-sensitive sign | Payload valuation/notional/price | TR valuation and amount response fields | UTI + valuation date | Numeric tolerance classes V1/N1/P1 | T+1 | Warning/Critical | Reg Ops + Finance Control | Procedure-validated (EU+UK+ETORO positions+trades+Seychelles) | Includes ISO/GBX handling, ETORO position valuation formula, ETORO trade notional derivation, and Seychelles valuation timestamp method |
...
| EMIR | EMIR EU/UK REFIT (+ Seychelles) | Collateralization flag | `EMIR2_Refit_Report_Daily.Uncollateralised`; `EMIR2_Seychelles_Refit_Report_Daily.Uncollateralised` | Derived in final insert logic from `Level` and specific counterparty list; Seychelles marks `TCTN` as empty and applies `TRUE/FALSE` rules for `PSTN` using fixed counterparty allowlist | Payload collateralization indicator | TR/state validation field | UTI + counterparty + level | Exact enum mapping | T+1 | Warning | Reg Ops + Compliance | Procedure-validated (EU+Seychelles) | EU and Seychelles both distinguish `TCTN` vs `PSTN`; Seychelles counterparty list includes `549300L7LPQNKJQ1IW32` |
...
| EMIR | 12 | 12 | Populated (EU+UK+ETORO positions+trades+Seychelles procedure-validated, endpoint validation pending) | Confirm REGIS/DTCC response code mapping and nullable behaviors; lock payload/response physical names |
