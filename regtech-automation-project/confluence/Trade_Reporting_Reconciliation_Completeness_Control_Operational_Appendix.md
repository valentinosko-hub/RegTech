...
| Regime | Audit filters (table.column = value set) | Primary comparison (used for completeness priority) | BI filter source tables and fields used for regime count | BI filter values (regime-specific) | TraNa filter fields (table.column) | TraNa filter values | BestEX diagnostic | BestEX filter fields (table.column) | BestEX filter values |
|---|---|---|---|---|---|---|---|---|---|
| MiFID UK CL | `MIFID2_Report.RegulationID=2`; `MIFID2_Report.RegulationReportID=2`; `MIFID2_Report.OpenORClose IN ('C','O')`; `MIFID2_Report.ReportDate BETWEEN @ReportDate1 AND @ReportDate2` | Audit reporting tables vs BI baseline tables | `Fact_SnapshotCustomer.RegulationID`; `Dim_Position.InstrumentID` / `Dim_Instrument.InstrumentTypeID`; `Dim_Position.IsSettled`; `Reg_Instruments_SCD.IsMifidByFCA`; `regulation_report_excluded_cids.cid`; `Dim_Range.FromDateID/ToDateID` | `RegulationID IN (2)`; `(InstrumentID IN (319,341) OR InstrumentTypeID IN (4,5,6))`; `IsMifidByFCA=1`; shared BI eligibility filters | `RegulationAggTrans.eToroEntity`; `.OpenORClose`; `.IsMifidByFCA`; `.IsMifidByESMA`; `.Trade_date` | `eToroEntity='eToro UK'`; `OpenORClose IN ('ClientOpen','ClientClose')`; `IsMifidByFCA=1`; `IsMifidByESMA IN (0,1)`; date in range | Yes (`MIFID2_Report` vs `BestEX_Report`) | `BestEX_Report.Trade_date/eToroEntity/OpenORClose/InstrumentID`; `Reg_Instruments_SCD.ValidFrom/ValidTo/IsMifidByFCA` | `Trade_date` in range; `eToroEntity='eToro UK'`; `OpenORClose IN ('C','O')`; date-valid SCD instrument with `IsMifidByFCA=1` |
| MiFID EU CL | `MIFID2_Report.RegulationID=1`; `MIFID2_Report.RegulationReportID=1`; `MIFID2_Report.OpenORClose IN ('C','O')`; `MIFID2_Report.ReportDate BETWEEN @ReportDate1 AND @ReportDate2` | Audit reporting tables vs BI baseline tables | `Fact_SnapshotCustomer.RegulationID`; `Dim_Position.InstrumentID` / `Dim_Instrument.InstrumentTypeID`; `Dim_Position.InstrumentID`; `Dim_Position.IsSettled`; `Reg_Instruments_SCD.IsMifid`; `regulation_report_excluded_cids.cid`; `Dim_Range.FromDateID/ToDateID` | `RegulationID IN (1)`; `(InstrumentID IN (319,341) OR InstrumentTypeID IN (4,5,6))`; `InstrumentID<>624`; `IsMifid=1`; shared BI eligibility filters | `RegulationAggTrans.eToroEntity`; `.OpenORClose`; `.IsMifidByESMA`; `.IsMifidByFCA`; `.InstrumentID`; `.Trade_date` | `eToroEntity='eToro EU'`; `OpenORClose IN ('ClientOpen','ClientClose')`; `IsMifidByESMA=1`; `IsMifidByFCA IN (0,1)`; `InstrumentID<>624`; date in range | Yes (`MIFID2_Report` vs `BestEX_Report`) | `BestEX_Report.Trade_date/eToroEntity/OpenORClose/InstrumentID`; `Reg_Instruments_SCD.ValidFrom/ValidTo/IsMifid` | `Trade_date` in range; `eToroEntity='eToro EU'`; `OpenORClose IN ('C','O')`; date-valid SCD instrument with `IsMifid=1`; `InstrumentID<>624` in missing-in-audit branch |
| MiFID EU Hedge | `MIFID2_Hedge_Report.RegulationReportID=1`; `MIFID2_Hedge_Report.ReportDate BETWEEN @ReportDate1 AND @ReportDate2` | Audit reporting tables vs BI hedge baseline | `Hedge.ExecutionLog.ExecutionTime/Success/Units/ProviderExecID/OrderState`; `Reg_Ext_LiquidityAccountID.eToroEntity`; `Reg_LiquidtyAcount_SCD.ValidFrom/ValidTo`; `Reg_Instruments_SCD.IsMifid` | `ExecutionTime` date in range; `Units>0`; `Success=1`; not(`ProviderExecID IS NULL` and `OrderState=4`); LP entity in EU/UK set; MiFID instrument scope | `RegulationAggTrans.eToroEntity`; `.OpenORClose`; `.IsMifidByESMA`; `.IsMifidByFCA`; `.InstrumentID`; `.Trade_date` | `eToroEntity IN ('eToro EU','eToro UK')`; `OpenORClose='HedgeExecution'`; `IsMifidByESMA=1`; `IsMifidByFCA IN (0,1)`; `InstrumentID<>624`; date in range | No | N/A | N/A |
| MiFID EU AUS | `MIFID2_ETORO_Report.RegulationID=1`; `MIFID2_ETORO_Report.RegulationReportID=1`; `MIFID2_ETORO_Report.OpenORClose IN ('C','O')`; `MIFID2_ETORO_Report.ReportDate BETWEEN @ReportDate1 AND @ReportDate2` | Audit reporting tables vs BI baseline tables | `Fact_SnapshotCustomer.RegulationID`; `Dim_Position.InstrumentID` / `Dim_Instrument.InstrumentTypeID`; `Dim_Position.InstrumentID`; `Dim_Position.IsSettled`; `Reg_Instruments_SCD.IsMifid`; `regulation_report_excluded_cids.cid`; `Dim_Range.FromDateID/ToDateID` | `RegulationID IN (4,10)`; MiFID instrument scope; `InstrumentID<>624`; `IsMifid=1`; `IsSettled=0`; shared BI eligibility filters | `RegulationAggTrans.eToroEntity`; `.[CFD/Real]`; `.OpenORClose`; `.IsMifidByESMA`; `.IsMifidByFCA`; `.InstrumentID`; `.Trade_date` | `eToroEntity='eToro AUS'`; `[CFD/Real]='CFD'`; `OpenORClose IN ('ClientOpen','ClientClose')`; `IsMifidByESMA=1`; `IsMifidByFCA IN (0,1)`; `InstrumentID<>624`; date in range | Yes (`MIFID2_ETORO_Report` vs `BestEX_Report`) | `BestEX_Report.Trade_date/eToroEntity/[CFD/Real]/OpenORClose/InstrumentID`; `Reg_Instruments_SCD.ValidFrom/ValidTo/IsMifid` | `Trade_date` in range; `eToroEntity='eToro AUS'`; `[CFD/Real]='CFD'`; `OpenORClose IN ('C','O')`; date-valid SCD instrument with `IsMifid=1`; `InstrumentID<>624` in missing-in-audit branch |
| MiFID EU SC | `MIFID2_Report.RegulationID=9`; `MIFID2_Report.RegulationReportID=1`; `MIFID2_Report.OpenORClose IN ('C','O')`; `MIFID2_Report.ReportDate BETWEEN @ReportDate1 AND @ReportDate2` | Audit reporting tables vs BI baseline tables | `Fact_SnapshotCustomer.RegulationID`; `Dim_Position.InstrumentID` / `Dim_Instrument.InstrumentTypeID`; `Dim_Position.InstrumentID`; `Dim_Position.IsSettled`; `Reg_Instruments_SCD.IsMifid`; `regulation_report_excluded_cids.cid`; `Dim_Range.FromDateID/ToDateID` | `RegulationID IN (9)`; MiFID instrument scope; `InstrumentID<>624`; `IsMifid=1`; shared BI eligibility filters | `RegulationAggTrans.eToroEntity`; `.OpenORClose`; `.IsMifidByESMA`; `.IsMifidByFCA`; `.InstrumentID`; `.Trade_date` | `eToroEntity='eToro SC'`; `OpenORClose IN ('ClientOpen','ClientClose')`; `IsMifidByESMA=1`; `IsMifidByFCA IN (0,1)`; `InstrumentID<>624`; date in range | Yes (`MIFID2_Report` vs `BestEX_Report`) | `BestEX_Report.Trade_date/eToroEntity/OpenORClose/InstrumentID`; `Reg_Instruments_SCD.ValidFrom/ValidTo/IsMifid` | `Trade_date` in range; `eToroEntity='eToro SC'`; `OpenORClose IN ('C','O')`; date-valid SCD instrument with `IsMifid=1`; `InstrumentID<>624` in missing-in-audit branch |
| MiFID EU ME | `MIFID2_ME_Report.RegulationID=11`; `MIFID2_ME_Report.RegulationReportID=1`; `MIFID2_ME_Report.OpenORClose IN ('C','O')`; `MIFID2_ME_Report.ReportDate BETWEEN @ReportDate1 AND @ReportDate2` | Audit reporting tables vs BI baseline tables | `Fact_SnapshotCustomer.RegulationID`; `Dim_Position.InstrumentID` / `Dim_Instrument.InstrumentTypeID`; `Dim_Position.InstrumentID`; `Dim_Position.IsSettled`; `Reg_Instruments_SCD.IsMifid`; `regulation_report_excluded_cids.cid`; `Dim_Range.FromDateID/ToDateID` | `RegulationID IN (11)`; MiFID instrument scope; `InstrumentID<>624`; `IsMifid=1`; shared BI eligibility filters | `RegulationAggTrans.eToroEntity`; `.OpenORClose`; `.IsMifidByESMA`; `.IsMifidByFCA`; `.InstrumentID`; `.Trade_date` | `eToroEntity='eToro ME'`; `OpenORClose IN ('ClientOpen','ClientClose')`; `IsMifidByESMA=1`; `IsMifidByFCA IN (0,1)`; `InstrumentID<>624`; date in range | Yes (`MIFID2_ME_Report` vs `BestEX_Report`) | `BestEX_Report.Trade_date/eToroEntity/OpenORClose/InstrumentID`; `Reg_Instruments_SCD.ValidFrom/ValidTo/IsMifid` | `Trade_date` in range; `eToroEntity='eToro ME'`; `OpenORClose IN ('C','O')`; date-valid SCD instrument with `IsMifid=1`; `InstrumentID<>624` in missing-in-audit branch |
| MiFID EU FCA | `MIFID2_Report.RegulationID=2`; `MIFID2_Report.RegulationReportID=1`; `MIFID2_Report.OpenORClose IN ('C','O')`; `MIFID2_Report.ReportDate BETWEEN @ReportDate1 AND @ReportDate2` | Audit reporting tables vs BI baseline tables | `Fact_SnapshotCustomer.RegulationID`; `Dim_Position.InstrumentID` / `Dim_Instrument.InstrumentTypeID`; `Reg_Instruments_SCD.IsMifid`; `Reg_Instruments_SCD.IsMifidByFCA`; `Dim_Position.IsSettled`; `regulation_report_excluded_cids.cid`; `Dim_Range.FromDateID/ToDateID` | `RegulationID IN (2)`; MiFID instrument scope; `IsMifid=1`; `IsMifidByFCA=1`; shared BI eligibility filters | `RegulationAggTrans.eToroEntity`; `.OpenORClose`; `.IsMifidByESMA`; `.IsMifidByFCA`; `.InstrumentID`; `.Trade_date` | `eToroEntity='eToro UK'`; `OpenORClose IN ('ClientOpen','ClientClose')`; `IsMifidByESMA=1`; `IsMifidByFCA IN (0,1)`; `InstrumentID<>624`; date in range | Yes (`MIFID2_Report` vs `BestEX_Report`) | `BestEX_Report.Trade_date/eToroEntity/OpenORClose/InstrumentID`; `Reg_Instruments_SCD.ValidFrom/ValidTo/IsMifid/IsMifidByFCA` | `Trade_date` in range; `eToroEntity='eToro UK'`; `OpenORClose IN ('C','O')`; date-valid SCD instrument with `IsMifid=1` and `IsMifidByFCA=1` |
| EMIR TR CL | `EMIR2_Refit_Report.Level='TCTN'`; `EMIR2_Refit_Report.RegulationID IN (1,2)`; `EMIR2_Refit_Report.FlippedReport=0`; `EMIR2_Refit_Report.OpenORClose='O'`; `ReportDate` in range | Audit reporting tables vs BI baseline tables | `Fact_SnapshotCustomer.RegulationID`; `Dim_Position` open-leg population (`OpenDateID` / `CloseDateID`); `Dim_Position.IsSettled`; `regulation_report_excluded_cids.cid`; `Dim_Range.FromDateID/ToDateID` | `RegulationID IN (1,2)`; open-leg scope; `IsSettled=0`; shared BI eligibility filters | `RegulationAggTrans.eToroEntity`; `.[CFD/Real]`; `.OpenORClose`; `.Trade_date` | `eToroEntity IN ('eToro EU','eToro UK')`; `[CFD/Real]='CFD'`; `OpenORClose='ClientOpen'`; date in range | Yes (`EMIR2_Refit_Report` vs `BestEX_Report`) | `BestEX_Report.Trade_date/eToroEntity/OpenORClose/[CFD/Real]` | `Trade_date` in range; `eToroEntity IN ('eToro EU','eToro UK')`; `OpenORClose='O'`; `[CFD/Real]='CFD'` |
| EMIR TR AUS | `EMIR2_ETORO_Refit_Trades.Level='TCTN'`; `RegulationID IN (4,10)`; `FlippedReport=0`; `OpenORClose='O'`; `ReportDate` in range | Audit reporting tables vs BI baseline tables | `Fact_SnapshotCustomer.RegulationID`; `Dim_Position` open-leg population (`OpenDateID` / `CloseDateID`); `Dim_Position.IsSettled`; `regulation_report_excluded_cids.cid`; `Dim_Range.FromDateID/ToDateID` | `RegulationID IN (4,10)`; open-leg scope; `IsSettled=0`; shared BI eligibility filters | `RegulationAggTrans.eToroEntity`; `.[CFD/Real]`; `.OpenORClose`; `.Trade_date` | `eToroEntity='eToro AUS'`; `[CFD/Real]='CFD'`; `OpenORClose='ClientOpen'`; date in range | Yes (`EMIR2_ETORO_Refit_Trades` vs `BestEX_Report`) | `BestEX_Report.Trade_date/eToroEntity/OpenORClose/[CFD/Real]` | `Trade_date` in range; `eToroEntity='eToro AUS'`; `OpenORClose='O'`; `[CFD/Real]='CFD'` |
| EMIR TR SC | `EMIR2_Refit_Report.Level='TCTN'`; `RegulationID=9`; `FlippedReport=0`; `OpenORClose='O'`; `ReportDate` in range | Audit reporting tables vs BI baseline tables | `Fact_SnapshotCustomer.RegulationID`; `Dim_Position` open-leg population (`OpenDateID` / `CloseDateID`); `Dim_Position.IsSettled`; `regulation_report_excluded_cids.cid`; `Dim_Range.FromDateID/ToDateID` | `RegulationID IN (9)`; open-leg scope; `IsSettled=0`; shared BI eligibility filters | `RegulationAggTrans.eToroEntity`; `.[CFD/Real]`; `.OpenORClose`; `.Trade_date` | `eToroEntity='eToro SC'`; `[CFD/Real]='CFD'`; `OpenORClose='ClientOpen'`; date in range | Yes (`EMIR2_Refit_Report` vs `BestEX_Report`) | `BestEX_Report.Trade_date/eToroEntity/OpenORClose/[CFD/Real]` | `Trade_date` in range; `eToroEntity='eToro SC'`; `OpenORClose='O'`; `[CFD/Real]='CFD'` |
| EMIR TR ME | `EMIR3_ME_Refit_Report.Level='TCTN'`; `RegulationID=11`; `FlippedReport=0`; `OpenORClose='O'`; `ReportDate` in range | Audit reporting tables vs BI baseline tables | `Fact_SnapshotCustomer.RegulationID`; `Dim_Position` open-leg population (`OpenDateID` / `CloseDateID`); `Dim_Position.IsSettled`; `regulation_report_excluded_cids.cid`; `Dim_Range.FromDateID/ToDateID` | `RegulationID IN (11)`; open-leg scope; `IsSettled=0`; shared BI eligibility filters | `RegulationAggTrans.eToroEntity`; `.[CFD/Real]`; `.OpenORClose`; `.Trade_date` | `eToroEntity='eToro ME'`; `[CFD/Real]='CFD'`; `OpenORClose='ClientOpen'`; date in range | Yes (`EMIR3_ME_Refit_Report` vs `BestEX_Report`) | `BestEX_Report.Trade_date/eToroEntity/OpenORClose/[CFD/Real]` | `Trade_date` in range; `eToroEntity='eToro ME'`; `OpenORClose='O'`; `[CFD/Real]='CFD'` |
| ASIC TR CL | `ASIC2_Transactions.RegulationID IN (4,10)`; `ASIC2_Transactions.OpenORClose IN ('O','C')`; `ReportDate` in range | Audit reporting tables vs BI baseline tables | `Fact_SnapshotCustomer.RegulationID`; `Dim_Position` open+close leg population (`OpenDateID` / `CloseDateID`); `Dim_Position.IsSettled`; `regulation_report_excluded_cids.cid`; `Dim_Range.FromDateID/ToDateID` | `RegulationID IN (4,10)`; open+close leg scope; `IsSettled=0`; shared BI eligibility filters | `RegulationAggTrans.eToroEntity`; `.[CFD/Real]`; `.OpenORClose`; `.Trade_date` | `eToroEntity='eToro AUS'`; `[CFD/Real]='CFD'`; `OpenORClose IN ('ClientOpen','ClientClose')`; date in range | Yes (`ASIC2_Transactions` vs `BestEX_Report`) | `BestEX_Report.Trade_date/eToroEntity/OpenORClose/[CFD/Real]` | `Trade_date` in range; `eToroEntity='eToro AUS'`; `OpenORClose IN ('O','C')`; `[CFD/Real]='CFD'` |
| ASIC TR EU | `ASIC2_Transactions.RegulationID IN (4,10)`; `ASIC2_Transactions.OpenORClose IN ('O','C')`; `ReportDate` in range | Audit reporting tables vs BI baseline tables | `Fact_SnapshotCustomer.RegulationID`; `Dim_Position` open+close leg population (`OpenDateID` / `CloseDateID`); `Dim_Position.IsSettled`; `regulation_report_excluded_cids.cid`; `Dim_Range.FromDateID/ToDateID` | `RegulationID IN (4,10)`; open+close leg scope; `IsSettled=0`; shared BI eligibility filters | `RegulationAggTrans.eToroEntity`; `.[CFD/Real]`; `.OpenORClose`; `.Trade_date` | `eToroEntity='eToro AUS'`; `[CFD/Real]='CFD'`; `OpenORClose IN ('ClientOpen','ClientClose')`; date in range | Yes (`ASIC2_Transactions` vs `BestEX_Report`) | `BestEX_Report.Trade_date/eToroEntity/OpenORClose/[CFD/Real]` | `Trade_date` in range; `eToroEntity='eToro AUS'`; `OpenORClose IN ('O','C')`; `[CFD/Real]='CFD'` |
| EMIR POS CL | `EMIR2_Refit_Report.ReportDate=@ReportDate1`; `EMIR2_Refit_Report.IsPosition=1`; `EMIR2_Refit_Report.FlippedReport=0`; `EMIR2_Refit_Report.RegulationID<>9` | Audit reporting tables vs BI baseline tables | `Fact_SnapshotCustomer.RegulationID`; active-position logic in `Dim_Position` (`OpenDateID<=ReportDateID` and `CloseDateID=0 OR >ReportDateID`); `Dim_Position.OpenOccurred`; `Dim_Position.IsSettled`; `regulation_report_excluded_cids.cid`; `Dim_Range.FromDateID/ToDateID` | `RegulationID IN (1,2)`; active position on report date; `OpenOccurred>='2014-02-12'`; `IsSettled=0`; shared BI eligibility filters | N/A (no TraNa comparator in SP) | N/A | No | N/A | N/A |
| ASIC POS CL | `ASIC2_Positions_AGG.ReportDate=@ReportDate1` | Audit reporting tables vs BI baseline tables | `Fact_SnapshotCustomer.RegulationID`; active-position logic in `Dim_Position` (`OpenDateID<=ReportDateID` and `CloseDateID=0 OR >ReportDateID`); `Dim_Position.IsSettled`; `regulation_report_excluded_cids.cid`; `Dim_Range.FromDateID/ToDateID` | `RegulationID IN (4,10)`; active position on report date; `IsSettled=0`; shared BI eligibility filters | N/A (no TraNa comparator in SP) | N/A | No | N/A | N/A |
