...
| EMIR TR CL | Audit reporting tables vs BI baseline tables | `Fact_SnapshotCustomer.RegulationID IN (1,2)`; `Dim_Position` open-leg population (`OpenDateID` / `CloseDateID`); `Dim_Position.IsSettled=0`; `regulation_report_excluded_cids.cid`; `Dim_Range.FromDateID/ToDateID` | Audit vs TraNa (`RegulationAggTrans`) | Yes (`EMIR2_Refit_Report` vs `BestEX_Report`) | `BestEX_Report.Trade_date/eToroEntity/OpenORClose/[CFD/Real]` | `Trade_date` in range; `eToroEntity IN ('eToro EU','eToro UK')`; `OpenORClose='O'`; `[CFD/Real]='CFD'` |
| EMIR TR AUS | Audit reporting tables vs BI baseline tables | `Fact_SnapshotCustomer.RegulationID IN (4,10)`; `Dim_Position` open-leg population (`OpenDateID` / `CloseDateID`); `Dim_Position.IsSettled=0`; `regulation_report_excluded_cids.cid`; `Dim_Range.FromDateID/ToDateID` | Audit vs TraNa (`RegulationAggTrans`) | Yes (`EMIR2_ETORO_Refit_Trades` vs `BestEX_Report`) | `BestEX_Report.Trade_date/eToroEntity/OpenORClose/[CFD/Real]` | `Trade_date` in range; `eToroEntity='eToro AUS'`; `OpenORClose='O'`; `[CFD/Real]='CFD'` |
| EMIR TR SC | Audit reporting tables vs BI baseline tables | `Fact_SnapshotCustomer.RegulationID IN (9)`; `Dim_Position` open-leg population (`OpenDateID` / `CloseDateID`); `Dim_Position.IsSettled=0`; `regulation_report_excluded_cids.cid`; `Dim_Range.FromDateID/ToDateID` | Audit vs TraNa (`RegulationAggTrans`) | Yes (`EMIR2_Refit_Report` vs `BestEX_Report`) | `BestEX_Report.Trade_date/eToroEntity/OpenORClose/[CFD/Real]` | `Trade_date` in range; `eToroEntity='eToro SC'`; `OpenORClose='O'`; `[CFD/Real]='CFD'` |
